...
- Native cross-platform execution verification (Linux/macOS/Windows) using the deterministic fake ACP harness: discovery, ACP initialize, the full stage path through integration/completion, run-workspace isolation, routing without sandbox/provider capability, cancellation, and probe/handshake timeouts. Descendant cleanup is best-effort and is never asserted as a security boundary.
