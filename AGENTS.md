...
- Native cross-platform execution verification (Linux/macOS/Windows) using the deterministic fake ACP harness: discovery, ACP initialize, the full stage path through integration/completion, run-workspace isolation, routing based on harness health and negotiated capability, cancellation, and probe/handshake timeouts. Descendant cleanup is best-effort.
