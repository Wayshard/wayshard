...
The crossed-path mark in `assets/branding/wayshard.png` is the canonical application icon artwork. Preserve its geometry, white/black treatment, cyan endpoint, and transparency. Use the same mark for browser favicons, installable Web icons, Linux, macOS, Windows, and Android rather than inventing platform-specific logos.
