...
func (r *Runner) execCheck(ctx context.Context, dir string, c artifacts.ValidationCheck) artifacts.ValidationCheck {
	...
	cmd.Dir = dir
	// Validation commands run as the Wayshard server OS user with the server
	// environment. Cancellation/timeout kills the process tree best-effort.
	cmd.Env = os.Environ()
