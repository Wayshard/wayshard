...
Stable ACP v1 is the initial protocol baseline. Keep protocol mechanics behind a driver and harness behavioral differences declarative in the catalog (`acp`, `interpose_commands`, `model_selection`) rather than in per-name Go adapters. Optional ACP features remain optional.
...
ACP terminal/tool callbacks are interposed by Wayshard, not executed by the harness: the harness asks the server to run a command and the server runs it as the server OS user. Interposition is a lifecycle/provenance boundary, not a sandbox. Discovered harnesses run with their normal configuration, authentication, environment, filesystem, and network access.

## Trusted local execution

Wayshard trusts the OS user running the server. There is no sandbox, no OS containment layer, no PID-namespace supervisor, no provider broker, and no fail-closed platform gate. This is a deliberate product decision: harnesses and tools run as the server OS user with their normal configuration, authentication, environment, filesystem, and network access on Linux, macOS, and Windows.

What Wayshard still owns: approvals (policy/UX, not containment), budgets, routing, isolated run workspaces, snapshots/checkpoints, server-owned validation and completion decisions, run-delta provenance, and conflict-safe integration. Process cleanup on cancellation/timeout/shutdown is best-effort; durable correctness never depends on killing a process.

## Discovery probing

Discovery runs installed executables normally as the server OS user for version and ACP-initialize probes, with a timeout, bounded combined output, and best-effort process-tree cleanup. The login-shell PATH probe reads the shell's required startup files and validates the returned PATH as an absolute-only, deduplicated, bounded list, never executing it as a command. A probe does not resolve harness-owned authentication, so auth state that cannot be determined is reported honestly (not assumed authenticated).
...
Wayshard trusts the server OS user. There is no OS containment layer: harnesses, ACP-requested tools, validation commands, and discovery probes all run as that user with their normal configuration, authentication, environment, filesystem, and network access.

Approvals are UX/policy, not containment. A denied approval means the protected server operation does not execute; it says nothing about an OS sandbox.

Do not require Docker and do not build one.

External file approval should usually import a read-only immutable snapshot rather than mount arbitrary host paths.

## OpenCode tool-interposition note

Wayshard interposes ACP terminal/fs callbacks: a harness asks the server to run a command and the server runs it as the server OS user in the run workspace. This gives server-owned lifecycle and provenance; it is not a sandbox. This is adapter/protocol behavior, not modification/installation of the user's OpenCode installation.
...
## Wayshard credentials

Wayshard-owned credentials (server identity private key, Jev/control-plane credentials, device material) live in restricted user config files: the credential directory is created `0700` and each file is written `0600` (user-profile ACL on Windows). Environment variables such as `TYPESAFE_API_KEY` are accepted and take precedence where applicable.

There is no mandatory SecretVault and no OS keyring, so the server runs on headless, container, and minimal systems. Do not store plaintext API keys in ordinary SQLite fields, logs, artifacts, or diagnostics; credential files are deliberately-restricted config, not database fields.
...
A crash can outlive a direct child. Process cleanup is best-effort (process group on Unix, direct child on Windows); recovery reconciles durable state (attempts, checkpoints, runs, publication journals) rather than process trees. A deliberately daemonizing process is accepted as a platform reality, and durable correctness never depends on killing it.
...
## Application icon decision

`assets/branding/wayshard.png` is the canonical raster application mark. All Web/PWA, Linux, macOS, Windows, and Android application icons derive from it; do not allow independently edited platform marks to drift. Preserve the artwork and alpha channel. Platform-only containers may use the Wayshard deep background (`#0e0f12`) and safe-area scaling where opaque or maskable icons require them.

Desktop PNG/ICNS/ICO and Web favicon/touch/PWA files are committed derivatives. Android's generated Gradle project is intentionally ephemeral in CI, so release automation generates its icons from `clients/desktop/src-tauri/app-icon.json` after Android initialization (`scripts/release/android-icons.sh`). The manifest points at the canonical mark for desktop/browser use and at the safe-area-padded `assets/branding/wayshard-android-fg.png` for the adaptive foreground and monochrome mask; `bg_color` supplies the deep background. Tauri's icon manifest needs tauri-cli >= 2.9.0; the pinned `@tauri-apps/cli` (2.11.5) is aligned with the locked `tauri` crate, so the same repo-pinned CLI drives `android-icons.sh`.
...
- Trusted local execution refactor (supersedes the containment/security architecture): Wayshard removed
  the Harness/Tool/Probe sandbox, `NetworkNone`, the provider-only network broker, the PID-namespace
  supervisor, process-tree capability gating, sandbox-based routing viability, the mandatory SecretVault
  and OS keyring, and fail-closed macOS/Windows execution. `internal/sandbox`, `internal/provider`, and
  `internal/secrets` were deleted; `internal/process` is now a small best-effort process-group helper.
  Discovered harnesses, ACP-requested tools, validation commands, and discovery probes run as the server
  OS user with their normal configuration, authentication, environment, filesystem, and network access on
  Linux, macOS, and Windows. Routing is harness/model selection over health and capability only. Wayshard
  credentials (server identity private key, Jev/control-plane credentials) live in restricted config files
  (`credentials/` directory 0700, files 0600; user-profile ACL on Windows) or environment variables
  (`TYPESAFE_API_KEY`), via `internal/credentials`; there is no vault and no keyring. Preserved: isolated
  run workspaces, snapshots/checkpoints, server-owned validation and completion, recovery, run-delta
  provenance, conflict-safe integration, ACP capability probing, timeouts, cancellation, best-effort
  process cleanup, routing, and harness-owned credentials.
...
- SQLite schema history: migration 004 added `process_owners` and `workspace_checkpoints.material_state`; migration 005 added `probe_owners`; migration 008 drops `process_owners`, `probe_owners`, and the harness/route `isolation`/provider-transport columns after the containment architecture was removed. Current schema version is 8.
...
- Wayshard no longer brokers or contains provider networking. Discovered harnesses use the server OS user's normal network. `internal/provider` and the provider shim were deleted; there is no broker, destination policy, or provider-only capability.
- Discovery/probing runs after `recovery.Reconcile`; probes run as the server OS user with a timeout, bounded output, and best-effort cleanup. Login-shell PATH discovery reads only per-shell startup files and validates the returned PATH.
- `GET /v1/sandbox` now reports the execution trust model (`trustModel: trusted_local`, `isolation: os_user`, `sandboxing: false`).
- `harness.harnessClosureFor` prepends a script/symlink harness's PATH-resolved interpreter directories to the launch environment (an environment convenience, not a filesystem boundary). The ACP driver supports `session/set_config_option`/`session/set_model`, and harnesses get the universal structured final-response contract.
- OpenCode's ACP needs a local loopback socket; it now simply uses the host loopback. A configured model can be selected, but harnesses normally advertise their own models.
...
- The catalog is a discovery/launch declaration, not a containment policy: there is no field for a sandbox because there is none. Well-known paths must be home-relative with no traversal, aliases are bare names and never package runners, unknown fields/enum values fail the entry closed, and catalog bytes/definitions/list entries/glob matches are bounded.
- `Definition.ExecutionFingerprint` is still a versioned SHA-256 over execution-relevant fields (executables, bridges, well-known, version/ACP/bridge args, native-vs-bridge, interposition, model selection, platforms). It is now used only to detect stale installations, not to gate provider trust.
- Well-known search roots must be home-relative; a search root that is a symlink escaping HOME is dropped, while a resolved executable symlink (NVM shim) is still discovered. (The former config-root/sensitive-location policy was removed with the sandbox.)
- Installations are reconciled, not accumulated: each row persists the effective definition's fingerprint, `Refresh` atomically replaces the persisted set, and `Candidates` excludes rows whose definition is missing/disabled/fingerprint-mismatched. Disabling, removing or materially changing a definition (or losing an executable/bridge) removes its route viability.
- `Discover` iterates the effective enabled definitions and resolves executable/bridge aliases over daemon PATH, sanitized login PATH, and Wayshard well-known bin dirs plus each definition's home-relative well-known dirs; there is no `switch definition.ID`. A route whose definition is missing is not routable.
- `GET /v1/harness-definitions` reports effective definitions (source/enabled) separately from `GET /v1/harnesses` installations (bridge path/presence, ACP status, blocking reason). A malformed/unsupported user catalog is surfaced through those diagnostics rather than log-only.
- TOML parsing uses `github.com/BurntSushi/toml` (per-entry diagnostics and duplicate-key rejection).
- Probes inherit the server environment and run the installed executables normally, so they may read real harness configuration; auth state that cannot be determined is still reported honestly.
...
- Historical (superseded by the trusted-local-execution refactor) Native runtime security milestone (after rc.8): added a typed sandbox capability model (`sandbox.Feature`, `RequiredFeatures`, `validateRequiredFeatures`) so a `Required` policy fails closed with `ErrRequiredIsolation` unless the backend declares every feature it depends on. **macOS** Seatbelt confinement is now natively verified on macOS 15 arm64 and `macos-15-intel` (filesystem read/write confinement, `NetworkNone` against a host loopback listener, process-group cancellation, synthetic HOME/TMPDIR, fail-before-exec); the profile is passed inline via `sandbox-exec -p` (no on-disk profile) and a relative command under `cmd.Dir` is resolved against `cmd.Dir`. **Windows** Job Objects are process/resource management only; filesystem/network/race-free-tree containment are not enforced, so `Report()` reports the required sandbox unavailable and required harness/tool/probe execution fails closed before any untrusted code runs — Windows stays a full server/client platform but local protected execution is refused. Isolation-unavailable maps to `FailPolicy` (run BLOCKED), not a retryable infrastructure failure. Probe ownership now fails closed for the server lifetime when an active owner cannot be verified (`recovery.ProbeOwnershipReconciled`). `native-security` CI jobs run the native black-box suite on macOS arm64/Intel and Windows. Known honest residual: a deliberately `setsid`-detached macOS descendant can leave the process group and survive cancellation, but it stays Seatbelt-confined and a stale owner blocks the run (fail closed). Native tests: `internal/sandbox/{feature,native_security}_test.go` (re-exec fixture), `internal/validation/tool_sandbox_test.go` now runs on macOS.
- Historical (superseded by the trusted-local-execution refactor) macOS process-tree ownership remediation (after the native-security pass): `FeatureProcessTree` is now backed by authoritative token-based ownership rather than a process group. `internal/process/owner_darwin.go` enumerates same-user processes and reads their initial environment via `sysctl(KERN_PROCARGS2)`, comparing the inherited `WAYSHARD_OWNER_TOKEN` digest so a `setsid`/`setpgid`/double-fork descendant is still found and killed and a reused PID cannot target an unrelated process; the capability is runtime-probed and, if unavailable, `FeatureProcessTree` is not advertised and required macOS execution fails closed. `finishProcessOwner` no longer marks an owner reconciled when ownership is unsupported (it leaves the owner active so recovery fails closed). `StoreProbeOwnerSink.BeginProbe` refuses to start a probe when ownership is unsupported, so no unreconcilable probe owner is created and a clean probe no longer disables future probing after restart. Tool sessions carry a distinct `WAYSHARD_TOOL_TOKEN` and reconcile escaped descendants on kill/release/close (via `process.ReconcileEnvToken`) without terminating the running harness; validation commands carry a per-command token and reconcile after completion/timeout. `process.ReconcileEnvToken` generalizes the Linux/darwin scanners to any env var name. Mandatory native tests (macOS arm64 + Intel): `TestNativeSetsidDescendantReconciled`, `TestNativeCancellationDoesNotKillUnrelated`, `TestDarwinFinishProcessOwnerReconcilesDetached`, `TestDarwinCleanProbeLeavesNoActiveOwner`, `TestDarwinDetachedProbeReconciledOnRestart`, `TestDarwinCleanProbeTwoStart`, `TestDarwinACPExecReconcilesProcessOwner`, `TestDarwinToolReleaseReconcilesDetached`, `TestDarwinValidationReconcilesDetached`; the native-security CI job fails if the macOS setsid/ownership tests skip.
- Historical (superseded by the trusted-local-execution refactor) macOS process ownership — adversarial token-stripping closure: environment ownership tokens (`WAYSHARD_OWNER_TOKEN` / `WAYSHARD_TOOL_TOKEN`) are NOT a security boundary. The untrusted process controls its children's exec environment, so it can strip the token, setsid away from the process group, and evade any `sysctl(KERN_PROCARGS2)` scan; reconciliation would then observe zero owned processes and could mark the durable owner reconciled while a stripped descendant survived. `kqueue` `EVFILT_PROC` + `NOTE_TRACK` is a real kernel fork-tracking primitive, but a PID-reuse-safe (`NOTE_TRACKERR`-safe, start-time-verified) implementation could not be demonstrated reliably within the pass, and a bug there could terminate an unrelated process (P0). Decision: **honest macOS fail-closed** — macOS no longer advertises `FeatureProcessTree`; `DarwinBackend.Report()` is `Available=false`, `Mode=seatbelt_no_ownership`; `ToolPolicy`/`HarnessPolicy`/`ProbePolicy`/`ReadOnlyViewPolicy`/validation all fail with `ErrRequiredIsolation` before untrusted code runs (`TestNativeRequiredPoliciesFailBeforeExec`, `TestNativeFailClosedBeforeExec`, `TestNativeValidationFailsClosed`, gated in the `native-security` CI job). Environment tokens are retained only as cooperative/diagnostic hints (and for Linux recovery); a durable owner is never reconciled merely because no token-bearing process was found on a platform where the token is removable. Linux remains the only platform advertising `FeatureProcessTree`. macOS stays a full Desktop/TUI/CLI/server/control-plane platform. The same environment-marker removability argument also applies to Linux and is recorded for a future Linux-ownership pass; Linux was explicitly out of scope here.
- Historical (superseded by the trusted-local-execution refactor) Linux process-lifecycle hardening (token-stripping closure): the same environment-token removability that closed macOS applies to Linux, so Linux no longer relies on tokens as ownership authority. Required execution (harness/tool/validation/probe, including provider and loopback policies) now runs under a trusted in-binary **PID-namespace supervisor** (`__wayshard-sandbox-supervise`) that remains namespace init while the target is its child. A process cannot leave its PID namespace, and killing namespace init tears every descendant down, so `KillTree` (kill the direct child + group) terminates setsid/double-forked descendants with no token scan. Server death is detected by a **death pipe** (parent holds the write end; EOF tears the namespace down) because namespace init sees `getppid() == 0` for an outside-namespace parent — the earlier getppid watchdog was wrong and killed the tree ~200ms after launch. `FeatureProcessTree` on Linux is advertised only when `procIsolationSupported()`; otherwise `Report()` is `Available=false` and required execution fails closed. A scoped procfs is mounted only when a policy requests `ProcIsolation`, so tools/validation still cannot read `/proc`. Tokens remain cooperative/diagnostic hints (and let recovery find a surviving supervisor). Adversarial native tests (`internal/sandbox/lifecycle_linux_test.go`) prove a token-stripped setsid descendant is terminated on cancellation, normal completion, server death, and under the loopback policy, and that token reconciliation spares an unrelated process; the app process-boundary orphan tests now assert supervisor teardown instead of orphan survival. Mandatory Linux CI gate added to the `native-security` job.
- Historical (superseded by the trusted-local-execution refactor) Ownership wording clarification (supersedes earlier token-authority wording): Linux process-tree termination authority is the trusted **PID-namespace supervisor** — killing it tears the namespace down. The per-attempt/tool/probe environment tokens (`WAYSHARD_OWNER_TOKEN`/`WAYSHARD_TOOL_TOKEN`) are auxiliary/diagnostic recovery hints only, used to locate a surviving supervisor and never treated as proof of ownership of a hostile process. Earlier entries that described `FeatureProcessTree` as "authoritative token-based ownership" (macOS) or reconciliation "by token" are historical and were superseded by the macOS honest fail-closed decision and by the Linux PID-namespace supervisor.
- Application icon wiring uses `assets/branding/wayshard.png` as the only artwork source. Desktop packages consume committed `clients/desktop/src-tauri/icons/{32x32.png,128x128.png,128x128@2x.png,icon.png,icon.icns,icon.ico}` derivatives; Web publishes the favicon, Apple touch, `any`, and maskable derivatives under `clients/web/public`; Android release CI runs `scripts/release/android-icons.sh` (repo-pinned tauri-cli, aligned with the locked `tauri` crate, driving `clients/desktop/src-tauri/app-icon.json`) after `tauri android init --ci`, with the safe-area-padded `assets/branding/wayshard-android-fg.png` as adaptive foreground and monochrome mask. `docs/assets/wayshard-app-icon-example.png` is the documented deep-background preview.
- Windows installer branding: `bundle.windows.nsis.installerIcon` in `clients/desktop/src-tauri/tauri.conf.json` points at `assets/branding/wayshard.ico` (9 BMP frames 16-256px, 32-bit, derived from the canonical mark). Without it NSIS falls back to its default installer icon. Tauri's NSIS template emits `!define MUI_ICON` from the canonicalized path, and `tauri build` chdirs to `src-tauri` so the relative path resolves to the repo root. NSIS copies the ICO frames into the PE verbatim; `scripts/release/verify-windows-installer-icon.py` fails the Windows release leg unless every frame is embedded, and `windows_installer_icon_test.sh` proves the gate with `makensis`.
- Pre-stable cleanup (rc.15): release Go builds are CGO-free (`CGO_ENABLED=0`), so Linux server/CLI are statically linked (host `linux/amd64` no longer picks up glibc); the server's `modernc.org/sqlite` is pure Go. `clients/web/public/social-share.png` (1200x630, canonical mark on `#0e0f12` with the wordmark) backs the `og:`/`twitter:` metadata and is asserted in the embedded Web build. Android signing is v2 + v3 only: `android-patch-gradle.py` sets `enableV1Signing = false` (minSdk 26 never uses v1/JAR; `apksigner verify` reports it false at that minSdk) and `package-android.sh` asserts v2/v3. `@tauri-apps/cli` is 2.11.5, aligned with the locked `tauri` 2.11.6 crate; `android-icons.sh` uses the repo-pinned CLI. The dead OpenCode `clients/ui/src/assets/images/social-share*.png` and the duplicate `clients/ui/src/assets/favicon/` tree were removed after proving no production references (no imports, not in `lineage.manifest.json`, not exported).
- Cancellation convergence (trusted-local follow-up): `orchestrator.ProcessRun` is now a wrapper over `processRun` that centralizes cancellation. If the run context is canceled or a cancellable operation returns `context.Canceled`/`context.DeadlineExceeded` (for example the run-status read right after a stage, which was the macOS/Windows flake), it performs the same durable `CancelRun` transition used by the in-loop checks and returns cleanly; a genuine non-cancellation error is still returned. Correctness does not depend on best-effort descendant cleanup. A test-only `processRunObserver` seam forces cancellation at exact vulnerable points (`status`, `assess`) so regression tests are deterministic rather than timing-based.
- Dependency security (pre-v0.2.0): triaged all open Dependabot alerts and recorded the methodology and decisions in `docs/dependency-security.md`. `golang.org/x/crypto` 0.45.0 → 0.52.0 (minimum safe; the 13 advisories are all in the unlinked `ssh` family, but they are criticals on a runtime module): this raised the Go directive to 1.25.0 and `golang.org/x/sys` to 0.45.0. `vite` 7.1.4 → 7.3.5 in `clients/web` (minimum safe; dev-server-only advisories). `glib` 0.18.5 (Tauri/gtk 0.18 transitive, Linux-desktop, unused by Wayshard Rust) is accepted and documented: its only patched version is 0.20.0, which requires a whole Tauri/wry/gtk-rs stack move, so it is not forced.
