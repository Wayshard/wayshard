...
- The supported-harness inventory is configuration, not code. The shipped catalog is `internal/harness/harnesses.toml` (embedded, `schema_version = 1`); the user catalog is `$XDG_CONFIG_HOME/wayshard/harnesses.toml` (per-platform config dir fallback). Merge is by stable `id`: user fields override shipped fields by key (arrays replace), `enabled = false` disables, deleting restores, user-only ids are custom, duplicates within one source are rejected, invalid entries are isolated, and unsupported schema versions/malformed TOML are rejected with diagnostics. Loaded at startup; a restart applies changes.
- The catalog cannot weaken containment: there is no field for host networking, disabling the sandbox/Landlock/seccomp, arbitrary environment, arbitrary host filesystem roots, approvals bypass, Tool/validation `NetworkNone` bypass, or trusting an unverified provider transport. Config/well-known paths must be home-relative (no absolute, no `..`), aliases are bare names and never package runners, and unknown fields/enum values fail the entry closed. Provider transport trust stays in the Wayshard-owned `verifiedProviderTransports` evidence registry.
...
- `GET /v1/harness-definitions` reports effective definitions (source/enabled) separately from `GET /v1/harnesses` installations (bridge path/presence, ACP status, blocking reason, provider transport). `GET /v1/sandbox` no longer claims provider networking is unavailable when `providerNetwork.available` is true; the always-on confinement report and runtime-probed provider/loopback capabilities are stated separately.
- SQLite schema version 6. Migration 006 adds harness-catalog discovery columns to `harness_installations` (`definition_source`, `bridge_executable`, `bridge_present`, `acp_status`, `blocking_reason`, `provider_transport`, `model_selection`, `requires_provider_network`).
- TOML parsing uses `github.com/BurntSushi/toml` (per-entry diagnostics and duplicate-key rejection).
- Known pre-existing flake (deferred to stabilization, unrelated to the harness catalog): `TestReclaimTerminalCheckpoint` in `internal/storage` can fail on Windows due to coarse clock/filesystem timing in the sub-millisecond reclaim-retention path.
