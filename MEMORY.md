...
- Application icon wiring uses `assets/branding/wayshard.png` as the only artwork source. Desktop packages consume committed `clients/desktop/src-tauri/icons/{32x32.png,128x128.png,128x128@2x.png,icon.png,icon.icns,icon.ico}` derivatives; Web publishes the favicon, Apple touch, `any`, and maskable derivatives under `clients/web/public`; Android release CI runs `scripts/release/android-icons.sh` (pinned manifest-capable tauri-cli driving `clients/desktop/src-tauri/app-icon.json`) after `tauri android init --ci`, with the safe-area-padded `assets/branding/wayshard-android-fg.png` as adaptive foreground and monochrome mask. `docs/assets/wayshard-app-icon-example.png` is the documented deep-background preview.
