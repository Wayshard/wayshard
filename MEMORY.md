...
- Pass 1E application-level port (continued): the narrow/mobile model is the upstream one — the layout context exposes `mobileSidebar` (opened/show/hide/toggle), the app-level titlebar carries the `xl:hidden` toggle, and `pages/layout/sidebar-mobile.tsx` renders the `data-component="sidebar-nav-mobile"` overlay with a scrim; the persistent sidebar is hidden below `xl` and the overlay hides on navigation. The behavior-bearing internals live in the adapted panels: Composer (`components/composer-region.tsx` + `components/session-composer-state.ts`), Review/Changes (`pages/session/review-tab.tsx`), Files (`pages/session/file-tabs.tsx`: Changes/All tab bar, tree, compare-and-set save) and Terminal (`pages/session/terminal-panel-v2.tsx`). `TestCancellationInterruptsActiveHarness` was made deterministic (load-tolerant budgets, cancellation wait with its own deadline, orphan check scoped to the test's built harness path). Routing provides real routes via an interim document-navigation bridge (`router.tsx`). Rendered smoke: `scripts/ci/ui_render_smoke.mjs`.
