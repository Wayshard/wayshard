...
Desktop PNG/ICNS/ICO and Web favicon/touch/PWA files are committed derivatives. Android's generated Gradle project is intentionally ephemeral in CI, so release automation runs the Tauri icon generator from `clients/desktop/src-tauri/app-icon.json` after Android initialization. The manifest points back to `assets/branding/wayshard.png`, supplies the adaptive background, and uses the same alpha silhouette as the Android monochrome mask.
...
- Application icon wiring uses `assets/branding/wayshard.png` as the only artwork source. Desktop packages consume committed `clients/desktop/src-tauri/icons/{32x32.png,128x128.png,128x128@2x.png,icon.png,icon.icns,icon.ico}` derivatives; Web publishes the favicon, Apple touch, `any`, and maskable derivatives under `clients/web/public`; Android release CI runs `bunx tauri icon src-tauri/app-icon.json` after `tauri android init --ci`. `docs/assets/wayshard-app-icon-example.png` is the documented deep-background preview.
