...
- OpenCode's ACP needs a local loopback socket, so the NetworkNone discovery probe reports it incompatible; its provider stage works. codex-acp initializes over real ACP but the current ChatGPT account is usage-limited for model calls.
