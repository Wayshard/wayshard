...
macOS uses `sandbox-exec`/Seatbelt with a compiled profile (filesystem read/write confinement and network denial), passed inline via `-p`; `sandbox-exec` absence fails closed. Whole-tree ownership is authoritative: an owned process is identified by the per-attempt `WAYSHARD_OWNER_TOKEN` it inherited (read through `sysctl(KERN_PROCARGS2)` for same-user processes), so a `setsid` descendant that escapes the process group is still found and terminated. `FeatureProcessTree` is advertised only when this capability is runtime-probed as available; otherwise the required sandbox is reported unavailable and required local execution fails closed. Native black-box tests on macOS arm64 and Intel verify the filesystem matrix, `NetworkNone`, setsid cancellation, tool-release and validation detached-child reconciliation, synthetic HOME/TMPDIR and fail-before-exec.
