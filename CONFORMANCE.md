...
- **Security is not user-configurable**: the catalog has no field to disable the sandbox, request host networking, inherit arbitrary environment, grant arbitrary host filesystem roots, bypass approvals, bypass Tool/validation `NetworkNone`, or mark an unverified provider transport as trusted. Paths must be home-relative, aliases must be bare names and never package runners (`npx`/`npm`/`bunx`/…), unknown fields and unsupported enum values fail the entry closed. Provider transport trust remains a Wayshard-owned evidence registry.
