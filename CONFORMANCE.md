...
| macOS runs harnesses/tools normally as the server OS user (Seatbelt removed; no platform gate) | `internal/harness`, `internal/validation` | cross-platform app tests | done |
| Windows runs harnesses/tools normally as the server OS user (Job Objects removed; no platform gate) | `internal/harness`, `internal/validation` | cross-platform app tests | done |
...
| Harness/tool/validation processes run in their own process group where supported and are cleaned up best-effort on cancel/timeout/shutdown | `internal/process`, `internal/harness/toolterm.go`, `internal/validation/discover.go` | `TestToolCancelKillsDescendants` | done |
