...
| Cancellation convergence: any cancellation observed during an active run (including a cancellation error escaping a storage or stage call) performs the same durable run/attempt/stage/approval cancellation transition before returning, so a run cannot be left non-terminal; correctness does not depend on best-effort process-tree cleanup | `internal/orchestrator/machine.go` (`ProcessRun`), `internal/storage/cancel.go` | `TestProcessRunCancellationDuringStatusReadConverges`, `TestProcessRunCancellationInsideStageConverges`, `TestProcessRunPreCanceledContextConverges`, `TestProcessRunPreservesNonCancellationFailure` | done |
